...
class SimpleGridLayout(Widget):
    ...
    def render(self):
        ...
        widget_counter = 0
        widget_limit = self._child_wigets.__len__()
        ...
        for row_index in range(self._number_of_rows):
            ...
            if self._rows_ratio.__len__() > 0\
                ...
                main_content += "<tr height=" + self._rows_ratio[row_index] + ">"
            else:
                main_content += "<tr>"
            ...
            for col_index in range(self._number_of_columns):
                ...
                if self._columns_ratio.__len__() > 0\
                    ...
                    main_content += "<td width=" + self._columns_ratio[col_index] + ">"
                else:
                    main_content += "<td>"
                # check if widget is available then render it
                if widget_counter < widget_limit:
                    main_content += self._child_wigets[widget_counter].render()
                    widget_counter += 1
                else:
                    main_content += ""
                main_content += "</td>"
...
class GridLayout(Widget):
    ...
    def render(self):
        ...
        for row_index in range(self._number_of_rows):
            if self._rows_ratio.__len__() > 0\
                    and row_index < self._rows_ratio.__len__():
                main_content += "<tr height=" + self._rows_ratio + ">"
            else:
                main_content += "<tr>"
            for col_index in range(self._number_of_columns):
                if self._columns_ratio.__len__() > 0\
                ...
                widget = self._child_wigets.get(key)
...
class VerticalLayout(Widget):
    ...
    def render(self):
        ...
        main_content = self._render_pre_content('div')
        for widget in self._child_wigets:
            main_content += "<div>" + widget.render() + "</div>"
        self._widget_content = main_content + self._render_post_content('div')
        return self._widget_content
...
class HorizontalLayout(Widget):
    ...
    def render(self):
        ...
        main_content += "<div style='display:table;'>"
        for widget in self._child_wigets:
            main_content += "<div style='display: table-cell;'>" + widget.render() + "</div>"
        main_content += "</div>"
        # table ended here
        self._widget_content = main_content + self._render_post_content('div')
        return self._widget_content
